...
from EigenTheda import xywhTheda2Eigen, Eigen2xywhTheda, Eigen2x, xywhTheda2Eigen_numpy, x2Eigen, Eigen2xywhTheda_numpy
...
def xywhTheda2Points(z):
    ...
    # rect = ((int(z[0]), int(z[1])), (int(z[2]), int(z[3])), int(180 - z[4]))
...
class LoadImagesAndLabels(Dataset):  # for training/testing
    ...
    def __getitem__(self, index):
        ...
        # if w(l) < h(s)
        temp_t = copy.deepcopy(labels)
        temp_wh = (temp_t[..., 3] < temp_t[..., 4])
        labels[temp_wh, 3] = temp_t[temp_wh, 4]
        labels[temp_wh, 4] = temp_t[temp_wh, 3]
        labels[temp_wh, 5] += 90
        labels[labels >= 180] -= 180

        # labels[:, -1] = labels[:, -1] / 180 * pi  # 0 ~ pi
        labels_Eigen = np.zeros((labels.shape[0], 6))  # x1, y1, x2, y2(0 - 1), angle(0 - 180)
        "[clsid cx cy l s θ] to [clsid, cx, cy, z1, z3, z2]"
        labels_Eigen[:, 0] = labels[:, 0]

        # if w(l) = h(s)
        labels[:, 3][labels[:, 3] / labels[:, 4] < 1.05] *= 1.05
        ee = xywhTheda2Eigen_numpy(copy.deepcopy(labels[:, 1:]))
        labels_Eigen[:, 1:] = Eigen2x(ee)

        nl = labels.shape[0]
        labels_out = torch.zeros((nl, 7))
        if nl:
            labels_out[:, 1:] = torch.from_numpy(labels_Eigen)
...
def P1P2P3P42xywhAngle(P1, P2, P3, P4, bound):
